...
namespace ConsoleApp2
{
    class Program
    {
        ...
        static void ShowListDirReq(ref int counter, string path, string depth = "-")
        {
            if (depth.Length == 1)
            {
                Console.WriteLine(path.Split('\\')[path.Split('\\').Length - 1]);
            }
        ...
        {
            if (path.Split('\\')[path.Split('\\').Length - 1] == dirname)
            {
                return path;
            }
            foreach (string dir in Directory.EnumerateDirectories(path))
...
Для того, чтобы удалить папку или файл используйте команду rm <путь> [ПАРАМЕТР]
   ...
   --- -r  - удаление файла или удаление папки и всех вложенных в неё папок и файлов
